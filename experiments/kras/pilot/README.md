...
The second Linux diagnostic run reproduced all normalized response entries within 8.89 × 10⁻¹⁵ and every ranking identically. Only near-zero polynomial-intermediate entry checks failed. After inspecting those diagnostics, the comparison scale was corrected as described above, without increasing the 10⁻¹⁰ dimensionless criterion. Comparing all 37 archived Linux model/result files under that policy passed; meaningful normalized perturbations and invalid variances are still rejected. The two earlier failed CI runs remain visible.
