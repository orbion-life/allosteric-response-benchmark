...
amendment=json.loads((root/'procedural-amendment.json').read_text())
consumed=amendment['first_attempt_wall_seconds'];remaining=1800-consumed
...
with (root/'execution.log').open('w') as log:
    ...
    while p.poll() is None:
        ...
        if time.monotonic()-start>remaining:reason='wall cap'
        ...
        time.sleep(.5)
    code=p.wait()
receipt={'exit_code':code,'watchdog_reason':reason,'seconds':time.monotonic()-start,'prior_attempt_seconds':consumed,'aggregate_attempt_seconds':consumed+time.monotonic()-start,'remaining_cap_seconds_at_start':remaining,'polled_peak_RSS_bytes':peak,'scope':'One local canary process; compilation uses num_processes=1'}
