...
def main():
    ...
    cases=[('zero',0,False),('positive_pi_over_two',2**(b-3),False),('negative_pi_over_two',2**b-2**(b-3),False),('coherent_two_pi_sign',0,True)]
    ...
    for label,word,coherent in cases:
        ...
        (ROOT/'results/full-bank-MPS-probes.json').write_text(json.dumps(dict(status='RUNNING' if len(records)<4 else 'PASS',scope='Numerical full69-qubit bank probes, fourdeclaredinputs; deterministicexpectations, nothardware/shotsorfull182-qubitestimator. MPSbond256/truncation1e-20 stated; no noise.',cases=records,wall_seconds=time.perf_counter()-start),indent=2)+'\n')
