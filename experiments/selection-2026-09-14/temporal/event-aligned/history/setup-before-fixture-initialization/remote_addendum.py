...
def campaign():
 ...
 for target in ['kras','abl','myc','myh7']:
  rows.append(run(target,Path('/results/event-aligned')/target));volume.commit()
 receipt={'wall_seconds':time.perf_counter()-start,'records':rows};Path('/results/event-aligned/campaign.json').write_text(json.dumps(receipt,indent=2));volume.commit();return receipt
