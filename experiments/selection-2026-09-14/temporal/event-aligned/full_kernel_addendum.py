...
def run(target,out,backend='cupy'):
 ...
 fixture,arrays=checks(xp);dump(out/'fixture-check.json',fixture);np.savez_compressed(out/'fixture-arrays.npz',**arrays)
 if not fixture['passed'] or fixture.get('cpu_gpu_passed') is False:raise ValueError('Independent CUDA fixture initialization/check failed')
 src=BASE/'inputs'/target;model=dict(np.load(src/'static-model.npz'));S=np.load(src/'fitted-covariance.npz')['covariance'];sd=np.load(src/'all-mode-harmonic-scales.npz')['harmonic_sd'];fixed=np.load(src/'fixed-operator.npz');tau=1/model['eigenvalues'][0]
