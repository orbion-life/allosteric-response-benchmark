...
The original saved-array verification and byte-identical waveform replay did not independently check the peak pair selector. Their passing status does not validate those withdrawn diagnostics. The corrected selector is tested with nonconsecutive odd/even receiver indices and an independent `np.take` extraction; see the erratum verification.
