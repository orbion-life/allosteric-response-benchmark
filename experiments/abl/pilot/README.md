...
The verifier uses an absolute tolerance of 10⁻¹⁰ for normalized responses, scores and equilibrium matrices. Integer, Boolean, rank and index values must match exactly. Its explicit list of raw means, centered features, residue covariance responses and standard deviations uses both absolute and relative tolerances of 10⁻¹⁰. Equal-time and time-lagged **monomial covariance** instead use a dimensionless check: each absolute difference is divided by the product of the corresponding monomial standard deviations from the **frozen equal-time covariance**, and must be at most 10⁻¹⁰. These fixed scales remain meaningful when an off-diagonal covariance vanishes by symmetry. Scales are never derived from the replay array. Nonpositive or nonfinite variances fail, as do changed shapes, dtypes or nonfinite array values. The tests reject altered standardized covariance, inflated replay variances, invalid scales and perturbed real response matrices, scores and rankings. This corrects verification policy without changing the model, inputs, numerical convergence criteria or scientific conclusions; see [the correction record](verifier-portability-correction.json).

New replay checks write [verification-current.json](checks/verification-current.json), including hashes of every reference and replay archive. The supplied current receipt checks all 12 downloaded Linux archives from CI run 34702338274; array values pass the stated tolerances, and every stored ranking is identical. The original macOS bitwise-equality receipt remains unchanged at [verification.json](checks/verification.json), with the original and intermediate verifiers and correction records retained in [history](history/verify-v0.3.0.py). The revised verifier has passed against the downloaded Linux output; a newly executed CI run remains a separate check. An archived output directory can be checked with `python verify.py --replay PATH`.
