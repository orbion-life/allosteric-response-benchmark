...
def run():
    ...
    for when in ['before','after']:
        for state in [0,1]:
            c=QuantumCircuit(6)
            if state:
                for q in range(6):c.r(np.pi,0,q)
            # Explicit native zero rotations keep |0> calibration a valid nonempty circuit.
            else:
                for q in range(6):c.r(0,0,q)
            lines=['OPENQASM 3.0;','bit[6] b;','#pragma braket verbatim','box {']
            for q,dev in enumerate([7,8,9,10,13,14]):lines.append(f'  prx({np.pi if state else 0:.17g}, 0) ${dev};')
            lines+=['}']+[f'b[{q}] = measure ${dev};' for q,dev in enumerate([7,8,9,10,13,14])]
            (out/'circuits'/f'calibration-{when}-state{state}.qasm').write_text('\n'.join(lines)+'\n')
            with (out/'circuits'/f'calibration-{when}-state{state}.qpy').open('wb') as f:qpy.dump(c,f)
    ...
    for shots in [16384,65536]:
        ...
        budgets.append(dict(shots_per_data_query=shots,total_data_shots=datashots,control_shots=ctrlshots,total_shots=datashots+ctrlshots,tasks=tasks,price_USD_at_verified_tariff=(datashots+ctrlshots)*.00145+tasks*.3,rigorous_statistical_response_halfwidth=wh,projection_error=projection,arithmetic_compile_allowance=1e-6,remaining_bias_allowance_for_0_05=.05-projection-1e-6-wh))
    target=.002;physical=.0005;numeric=1e-6;remaining=target-projection-physical-numeric;prob=remaining/(2*scale);n=math.ceil(math.log(2*J/.05)/(2*prob**2));ctrlshots=4*8192+3*16384;task=18*math.ceil(n/20000)+7
