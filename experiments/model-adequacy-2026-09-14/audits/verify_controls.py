...
Run python3 audits/verify_controls.py --output PATH. Writes only to that output directory.
"""
...
def main():
    ...
    changes={x['path']:x for x in json.loads((SOURCE/'PUBLIC-PACKAGING.json').read_text())['metadata_changes']}
    expected={}
    for name,h in frozen['input_hashes'].items():
        if name in changes:
            assert changes[name]['original_sha256']==h
            expected[name]=changes[name]['public_sha256']
        else: expected[name]=h
    mismatches=[name for name,h in expected.items() if digest(SOURCE/name)!=h]
    ...
    np.savez_compressed(OUT/'independent-control-moments.npz',**arrays,times_over_tau=z['times_over_tau'])
    receipt={'status':'PASS' if passed else 'FAIL','scope':'Fixed three-coordinate Gaussian and harmonic control evaluation only; no original nonlinear fidelity or protein claim',
             ...
             'source_hashes':{'audit_script':digest(__file__),'protocol.json':digest(SOURCE/'protocol.json'),
                             ...
                             'independent-control-moments.npz':digest(OUT/'independent-control-moments.npz')},
             'original_protocol_input_hashes':frozen['input_hashes'],'verified_public_input_hashes':expected,'metadata_redaction_count':len(changes),
             'environment':{'python':sys.version,'numpy':np.__version__,'platform':platform.platform()},'seconds':time.perf_counter()-start}
    (OUT/'controls-check.json').write_text(json.dumps(receipt,indent=2,allow_nan=False)+'\n')
...
if __name__=='__main__':
    ...
    OUT=args.output;OUT.mkdir(parents=True,exist_ok=True);main()
