...
from model import (ROOT, analytic_gaussian, build_model, contract, contract_moments,
...
def one_dimensional(A, h, diffusion, times):
    ...
    spectral = contract(Q.T, weighted)
    moments = []
    for t in [0., *times]:
        moments.append(contract(spectral.T, np.exp(-t*vals)[:, None]*spectral))
    first = contract(pi, P)
...
def independent_hermite(model):
    ...
    coefficients = contract(E.T, w[:, None]*He)/factorial[None, :]
    centered = np.sum(powers, axis=1) != 0
    coeff = coefficients[:, centered]
    factor = factorial[centered]
    powers = powers[centered]
    cov = contract(coeff*factor, coeff.T)
    sd = np.sqrt(np.diag(cov))
    delayed = []
    for t in model['times']:
        decay = np.exp(-t*model['mu']*contract(powers, model['eigenvalues']))
        delayed.append(contract(coeff*(factor*decay), coeff.T))
    norm = np.outer(sd, sd)
...
def small_full_grid(model, faces, h, sd):
    ...
    pi = np.exp(-V-logsumexp(-V))
    F = np.sqrt(pi)[:, None]*(E-contract(pi, E))/sd[None, :]
    G = len(x)
    ...
    G0 = contract(F.T, F)
    K = np.array([contract(F.T, expm_multiply(-t*H, F, traceA=-t*diag.sum())) for t in model['times']])
