...
def contract(a, b):
    """Explicit dense contraction avoids the warning-producing BLAS path."""
    signatures = {(1, 1): 'i,i->', (1, 2): 'i,ij->j',
                  (2, 1): 'ij,j->i', (2, 2): 'ij,jk->ik'}
    return np.einsum(signatures[(a.ndim, b.ndim)], a, b, optimize=False)
...
def energies(model, x):
    ...
    x = np.asarray(x)
    rr = model['r0'][None]+contract(x, model['Bx'].T).reshape(-1, 3, 3)
    E = np.zeros((len(x), 3))
    U = np.zeros(len(x))
...
def polynomial_values(model, x):
    ...
    return contract(monomials, model['coefficients'].T)
...
def contract_moments(model, moments, first):
    ...
    for k in range(3):
        ...
        means *= first[k][p[:, k]]
    m = contract(A, means)
    return contract(contract(A, kernel), A.T)-np.outer(m, m), m
...
def known_minimum(model):
    ...
    rebuilt = contract(model['basis'], q).reshape(3, 3)
