...
def main(stage, output):
    ...
    for name, args in jobs:
        ...
        with (output/(name+'.log')).open('w') as log:
            ...
            while child.poll() is None:
                ...
                time.sleep(.02)
            code = child.wait()
        receipt['jobs'].append(dict(name=name, return_code=code, sampled_worker_peak_bytes=sampled,
                                    outer_worker_wall_seconds=time.perf_counter()-job_started, watchdog_stop=killed))
    ...
    receipt.update(status='COMPLETE', wall_seconds=time.perf_counter()-started, sampled_aggregate_peak_bytes=peak,
                   wall_seconds_scope='Outer elapsed time from before the first fresh worker launch through the final worker exit and watchdog bookkeeping; includes each worker imports, receipt serialization and shutdown, excludes dependency installation and watchdog interpreter startup.')
    (output/'watchdog.json').write_text(json.dumps(receipt, indent=2)+'\n')
