...
def projected(H,F,V,times):
    ...
    # through the actual simulated Aused and its residual; no unclipped
    # residual is mixed with a clipped reduced trajectory.
    ...
    Aused=mm(U*lam_used,U.T)
    residual=HV-mm(V,Aused);Rgram=mm(residual.T,residual);Rgram=(Rgram+Rgram.T)/2
    ...
    # any nonzero V^T R. The mass-matrix term covers V^T V != I.
    # Coefficient norms, rather than reconstructed-vector norms, multiply both.
    mass_defect=mm(V.T,V)-np.eye(V.shape[1])
    ortho=float(np.max(abs(mass_defect)))
    mass_norm=float(np.linalg.norm(mass_defect,ord=2))
    coefficient_norms=np.linalg.norm(coeff,axis=0)
    ...
    for t in times:
        ...
        one=np.outer(norms,dnorms+np.sqrt(t*np.array(I)));one=np.minimum(one,one.T)
        two=initial+np.sqrt(np.outer(J,J))+(mass_norm+t*residual_parallel)*np.outer(coefficient_norms,coefficient_norms)
        ...
        response_bound=np.minimum(one,two)+static_error
        response_bound+=256*np.finfo(float).eps*(1+t*scale)*np.outer(norms,norms)
        one_bounds.append(one+static_error)
        two_bounds.append(two+static_error)
        unif=np.outer(norms,dnorms+t*normR*coefficient_norms)
        uniform.append(np.minimum(unif,unif.T)+static_error)
    ...
    row={'rank':V.shape[1],'orthogonality_error':ortho,'seed_projection_norm':float(np.linalg.norm(defect)),
         ...
         'residual_parallel_norm':residual_parallel,'mass_matrix_spectral_error':mass_norm,'max_response_bound':float(np.max(bounds)),
    ...
    if not reference_only:
        for index,(approx,row) in enumerate(zip(snapshots,records)):
            ...
            row.update(response_max_error=float(err.max()),delayed_max_error=float(delayerr.max()),minimum_bound_slack=float(np.min(approx['bound']-err)),
                       actual_error_within_calculated_bound=bool(np.all(err<=approx['bound']+protocol['gates']['bound_comparison_arithmetic_allowance'])),dimension_reduction=float(H.shape[0]/row['rank']))
