...
**Same-host verification status:** independent full replay review and parent verification are complete. The replay agrees across all 27 archives and 1,065 arrays, with maximum absolute difference 1.17×10⁻¹⁵. All selected ranks and scientific gate decisions agree; the combined practical count remains 16/18. The [independent review](audit/results-independent-review.md) and [parent verification](audit/parent-direct-verification.json) preserve the evidence and its scope. The study was published in [v0.4.0](https://github.com/orbion-life/allosteric-response-benchmark/releases/tag/v0.4.0). Version 0.4.1 corrects documentation only and adds proposed recovery designs. The known strict Linux bound-comparison failure below remains unresolved.
...
The repository workflow installs Python 3.12 and the pinned requirements, runs the seven original operator tests plus seven comparator tests with warnings treated as errors, and executes a fresh bounded campaign in a unique temporary directory. It retains generated arrays, diagnostics and failure logs as workflow artifacts. The [first complete Linux run](https://github.com/orbion-life/allosteric-response-benchmark/actions/runs/34717448039) completed all 27 cases and passed all 14 unit tests, but the unchanged strict comparison **failed**. Its [complete arrays and original failure receipt](audit/linux-replay-34717448039/) are retained.
...
- [`LICENSE`](LICENSE) governs this source package. The completed evidence release is [v0.4.0](https://github.com/orbion-life/allosteric-response-benchmark/releases/tag/v0.4.0); that release retains a known strict portability failure.
...
## Known cross-platform numerical limitation

The original pre-release Linux run (34717448039) fails the unchanged strict checker because **61 of 372 bound arrays** differ from the macOS primary run by more than the declared absolute 1e-9 allowance. All **414 response/covariance/scale/time arrays** agree within 5.3191e-10, and both runs retain the same selected ranks, 17/18 finite-fidelity decisions and failed grid/domain decisions. These are separate facts; response agreement does not make the strict comparison pass.

The projected matrices are not related only by a basis rotation: an examined case has different projected and residual spectra. The numerical Krylov subspaces can therefore differ across linear-algebra backends even when the queried responses remain close. The cause and general stability of this sensitivity require further numerical work. We do not loosen the comparison allowance, alter the pre-execution protocol or replace stored outputs after observing it.

Linux completes the campaign in 348.15 s, with 542,924,800 bytes of aggregate sampled memory. Its combined fidelity-and-cost count is 17/18, compared with 16/18 in each of the two macOS runs; cost alone is 18/18 on Linux. The narrow timing failure is consequently not portable. These are single cold campaigns, not estimates of runtime distributions. See the [machine-readable summary](audit/linux-portability-summary.json) and the [independent numerical diagnosis](audit/linux-portability-diagnosis.md), alongside the original FAIL receipt. This is a research evidence release with a known strict portability failure, not a claim that all CI checks pass.

The exact v0.4.0 main-branch replay (34718329506) has **62/372** failed bound comparisons; the v0.4.0 tag replay (34718353496) has **61/372**. All 414 response/covariance/scale/time arrays pass in both. These are distinct executions of the same release commit, not interchangeable counts. Their preserved outputs are in the [v0.4.0 release CI evidence](https://github.com/orbion-life/allosteric-response-benchmark/releases/download/v0.4.0/v0.4.0-release-ci-evidence.zip). The [proposed stability diagnostic](../../docs/recovery/subspace-stability-recovery.md) will trace identical input matrices and every admitted basis block; it has not yet run.
