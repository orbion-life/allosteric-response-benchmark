...
def run(output, workers=4, resume=False):
    ...
    if resume:
        ...
        receipt['prior_seconds']=old.get('prior_seconds',0)+max(0,(datetime.datetime.now(datetime.timezone.utc)-datetime.datetime.fromisoformat(old['started_utc'])).total_seconds())
    ...
    receipt.setdefault('prior_seconds',0)
    remaining=execution['cumulative_original_plus_continuation_cap_seconds']-receipt['prior_seconds']
    if remaining<=0:raise TimeoutError('Cumulative execution allocation exhausted before continuation.')
    ...
    with ProcessPoolExecutor(max_workers=workers) as pool:
        ...
        try:
            for future in as_completed(pending,timeout=min(execution['max_seconds_per_execution'],remaining)-(time.monotonic()-started)):
                name,meta=future.result()
                parent_peak=resource.getrusage(resource.RUSAGE_SELF).ru_maxrss*(1 if sys.platform=='darwin' else 1024)
                assert meta['worker_peak_bytes']*workers+parent_peak<=execution['peak_total_budget_bytes']
                receipt['cases'].append(meta)
                (output/'run.json').write_text(json.dumps(receipt,indent=2)+'\n')
            ...
            raise
    parent_peak=resource.getrusage(resource.RUSAGE_SELF).ru_maxrss*(1 if sys.platform=='darwin' else 1024)
    worker_peak=max([r.get('worker_peak_bytes',0) for r in receipt['cases']]+[0])
    receipt.update(status='COMPLETE',seconds=time.monotonic()-started,cumulative_seconds=receipt['prior_seconds']+time.monotonic()-started,parent_peak_bytes=parent_peak,max_worker_peak_bytes=worker_peak,conservative_concurrent_peak_bound_bytes=parent_peak+workers*worker_peak,completed_utc=datetime.datetime.now(datetime.timezone.utc).isoformat())
    (output/'run.json').write_text(json.dumps(receipt,indent=2)+'\n')
