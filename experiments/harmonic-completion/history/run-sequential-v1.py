        ...
        for d in protocol['retained_dimensions']:
            ...
            for n,extent in cases:
                for kind in ['biquadratic','harmonic']:
                    if time.monotonic()-started > protocol['compute_cap']['seconds_total']:
                        raise TimeoutError('Locked total compute cap reached; preserve completed cases.')
                    peak=resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
                    if sys.platform!='darwin':peak*=1024
                    if peak>protocol['compute_cap']['peak_bytes']:
                        raise MemoryError('Locked peak memory cap reached.')
